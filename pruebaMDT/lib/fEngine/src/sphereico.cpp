...
SphereIco::SphereIco(float radius, int sub, bool smooth) : radius(radius),
                                                           subdivision(sub),
                                                           smooth(smooth),
                                                           interleavedStride(32)
{
    if(smooth)
        ...
        buildVerticesFlat();
    m_distance_between_points =  getDistanceVertices();
...
void SphereIco::setInfluenceVerticesInRadius(float lat, float lon, float radius_influence)
{
    float x = radius * cos(lat*M_PI/360.0) *  cos(lon*M_PI/360.0);
    float y = radius * sin(lat*M_PI/360.0) *  sin(lon*M_PI/360.0);
    float z = radius * sin(lon*M_PI/360.0);
    ...
    float real_distance;
    vertices_in_influence.clear();
    //cuales puntos están mas cerca de la lat y long dentro de un radio radius
    for (unsigned long long i =0; i<vertices.size()/3; i++)
    {
        tmp_vector.push_back(vertices[i*3 + 0]);
        tmp_vector.push_back(vertices[i*3 + 1]);
        tmp_vector.push_back(vertices[i*3 + 2]);
        real_distance = getDistanceBetweenTwoPoints(reference, tmp_vector);
        if (real_distance < radius_influence)
        {
            vertices_in_influence.push_back(vertices[i*3 + 0]) ;
            vertices_in_influence.push_back(vertices[i*3 + 1]) ;
            vertices_in_influence.push_back(vertices[i*3 + 2]) ;
        }
        tmp_vector.clear();
    }
...
void SphereIco::computeFaceNormal(const float v1[3], const float v2[3], const float v3[3], float n[3])
{
...
void SphereIco::computeVertexNormal(const float v[], float normal[])
{
    // normalize
    float scale = SphereIco::computeScaleForLength(v, 1);
    normal[0] = v[0] * scale;
    normal[1] = v[1] * scale;
    normal[2] = v[2] * scale;
}

float SphereIco::computeScaleForLength(const float v[3], float length)
{
    // and normalize the vector then re-scale to new radius
    return length / sqrtf(v[0] * v[0] + v[1] * v[1] + v[2] * v[2]);
}

void SphereIco::buildVerticesFlat()
{
    ...
    for(int i = 1; i <= 5; ++i)
    {
        ...
        SphereIco::computeFaceNormal(v0, v1, v2, n);
        ...
        SphereIco::computeFaceNormal(v1, v3, v2, n);
        addVertices(v1, v3, v2);
        addNormals(n, n, n);
        addTexCoords(t1, t3, t2);
        addIndices(index+3, index+4, index+5);

        SphereIco::computeFaceNormal(v2, v3, v4, n);
        ...
        SphereIco::computeFaceNormal(v3, v11, v4, n);
...
void SphereIco::buildVerticesFlatPolar()
{
    for (unsigned long long i=0; i<vertices.size();i++)
    {
        ...
        vertices_geo.push_back(atan(z/y)*360/M_PI);
        vertices_geo.push_back(atan(y/x)*360/M_PI);
    }
}
void SphereIco::buildVerticesSmooth()
{
    //const float S_STEP = 1 / 11.0f;         // horizontal texture step
    //const float T_STEP = 1 / 3.0f;          // vertical texture step
    const float S_STEP = 186 / 2048.0f;     // horizontal texture step
    ...
    SphereIco::computeVertexNormal(v, n);
    ...
    scale = SphereIco::computeScaleForLength(v, 1);
...
void SphereIco::subdivideVerticesSmooth()
{
...
std::vector<float> SphereIco::computeIcosahedronVertices()
{
...
void SphereIco::subdivideVerticesFlat()
{
...
void SphereIco::buildInterleavedVertices()
{
...
float SphereIco::getDistanceBetweenTwoPoints(std::vector<float> reference1, std::vector<float> reference2)
{
    float dx = reference1[0] -  reference2[0];
    float dy = reference1[1] -  reference2[1];
    float dz = reference1[2] -  reference2[2];
    return sqrt(pow(dx,2) + pow(dy,2) + pow(dz,2));
}
void SphereIco::addVertex(float x, float y, float z)
{
...
void SphereIco::addNormal(float nx, float ny, float nz)
{
...
void SphereIco::addTexCoord(float s, float t)
{
    texCoords.push_back(s);
    texCoords.push_back(t);
}

void SphereIco::addVertices(const float v1[3], const float v2[3], const float v3[3])
{
...
void SphereIco::addNormals(const float n1[3], const float n2[3], const float n3[3])
{
...
void SphereIco::addTexCoords(const float t1[2], const float t2[2], const float t3[2])
{
...
void SphereIco::addIndices(unsigned int i1, unsigned int i2, unsigned int i3)
{
...
void SphereIco::computeHalfVertex(const float v1[3], const float v2[3], float length, float newV[3])
{
    newV[0] = v1[0] + v2[0];
    newV[1] = v1[1] + v2[1];
    newV[2] = v1[2] + v2[2];
    float scale = SphereIco::computeScaleForLength(newV, length);
    newV[0] *= scale;
    newV[1] *= scale;
    newV[2] *= scale;
}

void SphereIco::computeHalfTexCoord(const float t1[2], const float t2[2], float newT[2])
{
    newT[0] = (t1[0] + t2[0]) * 0.5f;
    newT[1] = (t1[1] + t2[1]) * 0.5f;
}

void SphereIco::addSubLineIndices(unsigned int i1,
                                  unsigned int i2,
                                  unsigned int i3,
                                  unsigned int i4,
                                  unsigned int i5,
...
unsigned int SphereIco::addSubVertexAttribs(const float v[3], const float n[3], const float t[2])
{
    unsigned int index;     // return value;

    // check if is shared vertex or not first
    if(SphereIco::isSharedTexCoord(t))
    {
...
bool SphereIco::isSharedTexCoord(const float t[2])
{
    ...
    {
        if(SphereIco::isOnLineSegment(&segments[i], &segments[j], t))
...
bool SphereIco::isOnLineSegment(const float a[2], const float b[2], const float c[2])
{
